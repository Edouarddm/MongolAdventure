...
import sonLines from "../content/sonDialog.js";
...
export default async function dungeon(k) {
  ...
  entities.player.onCollide("prison-door", async (prisonDoor) => {
    await dialog(
      k,
      k.vec2(250, 500),
      sonLines[gameState.getLocal()][playerState.getHasKey() ? 1 : 0],
    );
  ...
  entities.player.onCollide("son", async () => {
    await dialog(
      k,
      k.vec2(250, 500),
      sonLines[gameState.getLocal()][2],
    );
  });

  if (entities.ghost) {
    setGhostAI(k, entities.ghost, entities.player);
    onAttacked(k, entities.ghost, entities);
